...
- Nome: Analice Carvalho Fernandes
- Turma: 3B1
...
- Elas herdam da superClasse os campos id, data_criacao e data_atualizacao.

**5.** Qual é o `__tablename__` da tabela de favoritos? Por que usamos `__tablename__` em vez de só o nome da classe?
- filme_favoritos, usamos tablename para que o próprio sistema entenda qual o nome da tabela.
...
- A coluna qua guarda o id é: tmdb.id, ela possui a restrição "unique".
...
- Ele registra os dados de um filme que esta sendo adicionado agora no programa(tmdb_id, titulo, poster_path, nota e ano) além de, caso o filme já exista nos favoritos ele não será adicionado, RETURN.

**8.** Onde está o método que lista as últimas 8 buscas? Qual é o nome da classe e do método?
- Está em models/historico_busca.py. A classe "HistoricoBusca(ModelsBase)", métodos  classemethod.

**9.** O model grava dados da API TMDB inteira ou só alguns campos espelhados? Cite 4 campos salvos em `FilmeFavorito`.
- A classe FilmeFavorito armazena apenas campos selecionados da API TMDB, não a totalidade dos dados. A estrutura usa atributos como tmdb_id, titulo, poster_path, nota e ano.
...
- É exportado além de db: ModelsBase, FilmesFavoritos e HistoricoBusca. Não sei.
...
- Os Blueprints que existem são: dashboard, favoritos(/favoritos) e filmes(/filmes).

**12.** Em qual arquivo está a rota `/filmes/populares`? Qual é o nome da função Python que responde essa URL?
- Em filmes_favoritos.py, respondida por def populares().
...
- O controller "HistóricoBusca.registrar(termo, len)
