...
# TODO ALUNO: configure a URI do SQLite (arquivo loja.db nesta pasta) - CORRIGIDO
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + os.path.join(pasta_aula, "loja.db")
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False

# TODO ALUNO: crie o objeto db - CORRIGIDO
db = SQLAlchemy(app)


# --- MODEL: tabela produtos (COMPLETE A CLASSE) ---
class Produto(db.Model):
    # TODO ALUNO: nome da tabela - CORRIGIDO
    __tablename__ = "produto"   

    # TODO ALUNO: id (Integer, primary_key=True) - CORRIGIDO
...
# TODO ALUNO: criar tabelas no banco - CORRIGIDO
...
@app.route("/")
def index():
    # TODO ALUNO: busque todos os produtos ordenados por nome - CORRIGIDO (Removida a linha que zerava a lista)
...
def cadastrar():
    if request.method == "POST":
        ...
        # TODO ALUNO: add + commit - CORRIGIDO (comit -> commit)
...
def editar(produto_id):
    # TODO ALUNO: buscar produto pelo id - CORRIGIDO (Removida a linha que definia como None)
    ...
    if request.method == "POST":
        ...
        # TODO ALUNO: commit - CORRIGIDO (comit -> commit)
        db.session.commit()
        return redirect(url_for("index"))
